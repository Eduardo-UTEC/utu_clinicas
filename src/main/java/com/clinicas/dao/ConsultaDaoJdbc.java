...
public class ConsultaDaoJdbc implements ConsultaDao {
	public int upsert(Consulta c) throws Exception {
		...
		try (Connection cn = DB.getConnection(); PreparedStatement ps = cn.prepareStatement(UPD)) {
			ps.setString(1, c.getSintomas());
			ps.setInt(2, c.getNivelEmergencia());
			ps.setInt(3, c.getIdReserva());
		...
		try (Connection cn = DB.getConnection();
			...
			ps.setInt(3, c.getNivelEmergencia());
			int n = ps.executeUpdate();
		...
		try (Connection cn = DB.getConnection(); PreparedStatement ps = cn.prepareStatement(SQL)) {
			...
			try (ResultSet rs = ps.executeQuery()) {
				if (rs.next()) {
					...
					c.setNivelEmergencia(rs.getInt("Nivel_Emergencia"));
					return c;
				}
			}
		}
