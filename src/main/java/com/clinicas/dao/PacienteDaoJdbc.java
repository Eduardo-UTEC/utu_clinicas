    ...
    public int insert(Paciente p) throws Exception {
        ...
        try (Connection cn = DB.getConnection();
            ...
            try (ResultSet rs = ps.getGeneratedKeys()) {
                if (rs.next()) p.setIdPaciente(rs.getInt(1));
            }
            return n;
        } catch (SQLIntegrityConstraintViolationException e) {
            String msg = e.getMessage() == null ? "" : e.getMessage().toLowerCase();
            if (msg.contains("cedula")) throw new RuntimeException("Cédula ya registrada");
            if (msg.contains("email"))  throw new RuntimeException("EMail ya registrado");
            throw e;
        }
    }
