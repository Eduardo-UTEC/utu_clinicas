...
public class DoctorTurnoDaoJdbc implements DoctorTurnoDao{
  public void add(int idDoctor,int idTurno) throws Exception{
    ...
    try(Connection cn=DB.getConnection(); PreparedStatement ps=cn.prepareStatement(SQL)){ ps.setInt(1,idDoctor); ps.setInt(2,idTurno); ps.executeUpdate(); }
    catch(SQLIntegrityConstraintViolationException e){ throw new RuntimeException("Turno ya registrado para el doctor."); }
  }
  ...
  public List<Turno> findByDoctor(int idDoctor) throws Exception{
    ...
    try(Connection cn=DB.getConnection(); PreparedStatement ps=cn.prepareStatement(SQL)){ ps.setInt(1,idDoctor); try(ResultSet rs=ps.executeQuery()){ while(rs.next()){ Turno t=new Turno(); t.setIdTurno(rs.getInt("idTurno")); t.setNombre(rs.getString("nombre")); list.add(t);} } }
    return list;
  }
}
