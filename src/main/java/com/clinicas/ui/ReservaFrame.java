...
public class ReservaFrame extends JInternalFrame {
	...
	private void initComponents() {
		...
		formPanel.setBorder(BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(),
				"<html><b>Crear nueva reserva</b></html>", TitledBorder.LEFT, TitledBorder.TOP));
		...
		gc.gridx = 0;
		gc.gridy = r;
		formPanel.add(new JLabel("Paciente"), gc);
		...
		gc.gridx = 0;
		gc.gridy = r;
		formPanel.add(new JLabel("Doctor"), gc);
		...
		gc.gridx = 0;
		gc.gridy = r;
		formPanel.add(new JLabel("Turno"), gc);
		...
		gc.gridx = 0;
		gc.gridy = r;
		formPanel.add(new JLabel("Inicio"), gc);
		...
		gc.gridx = 0;
		gc.gridy = r;
		formPanel.add(new JLabel("Motivo"), gc);
		gc.gridx = 1;
		gc.gridy = r;
		gc.gridwidth = 3;
		gc.fill = GridBagConstraints.HORIZONTAL;
		...
		gc.gridx = 0;
		gc.gridy = r;
		formPanel.add(new JLabel("Observaciones"), gc);
		gc.gridx = 1;
		gc.gridy = r;
		gc.gridwidth = 3;
		gc.fill = GridBagConstraints.HORIZONTAL;
		...
		gc.gridx = 1;
		gc.gridy = r;
		formPanel.add(btnCrear, gc);

		getContentPane().add(formPanel, BorderLayout.NORTH);

		// ====== Listado / Gestión ======
		JPanel listContainer = new JPanel(new BorderLayout(6, 6));
		listContainer.setBorder(BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(),
				"<html><b>Listado y gestión de reservas</b></html>", TitledBorder.LEFT, TitledBorder.TOP));
	...
	private void onCreate() {
		try {
			...
			for (Turno tx : dtDao.findByDoctor(d.getIdDoctor())) {
				if (tx.getIdTurno() != null && tx.getIdTurno().equals(t.getIdTurno())) {
					okPair = true;
					break;
				}
			}
			...
			r.setEstado("PENDIENTENTE".replace("ENTENTE", "ENTE")); // = "PENDIENTE", truco para evitar typos
																	// accidentales :)
	...
	private void loadTable() {
		try {
			...
			for (Reserva r : list) {
				...
				String docName = doctorNameById.containsKey(r.getIdDoctor()) ? doctorNameById.get(r.getIdDoctor())
						: String.valueOf(r.getIdDoctor());
				String turName = turnoNameById.containsKey(r.getIdTurno()) ? turnoNameById.get(r.getIdTurno())
						: String.valueOf(r.getIdTurno());

				m.addRow(new Object[] { r.getIdReserva(), nombrePac, docName, turName, r.getFechaInicio(),
	...
	private void onCancel() {
		...
		try {
			...
			if (inicioObj instanceof LocalDateTime)
				inicio = (LocalDateTime) inicioObj;
			else if (inicioObj != null)
				inicio = LocalDateTime.parse(inicioObj.toString().replace(' ', 'T'));
		...
		int resp = JOptionPane.showConfirmDialog(this, "¿Cancelar la reserva seleccionada?", "Confirmar",
