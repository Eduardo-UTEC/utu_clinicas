...
/**
 * Programa un recordatorio por mail 2 hs antes de la reserva.
 * También hace un escaneo periódico cada 60s para agendar envíos pendientes.
 * Sin cambios de esquema: todo en memoria mientras la app esté abierta.
 */
public class ReminderScheduler {
    ...
    public static ReminderScheduler get(){ return INSTANCE; }

    /** Llamar una vez desde MainApp, pasando la implementación de EmailService. */
    public void start(EmailService emailService){
        this.email = emailService;
        // Escaneo periódico para agendar envíos (por si se agregan reservas mientras la app corre)
    ...
    public void scheduleFor(Reserva r){
        if(r==null || r.getIdReserva()==null) return;
        try{
    ...
    private void scheduleInternal(Reserva r, Paciente p, Doctor d){
        if(p==null || d==null || p.getEmail()==null || p.getEmail().isBlank()) return;
        ...
        Runnable task = () -> {
            try{
                ...
                String cuerpo  = "Estimado/a " + safeName(p) + ",\n\n"
                    + "Le recordamos su consulta con el Dr./Dra. " + safeName(d)
                    + " el " + r.getFechaInicio() + ".\n\nSaludos.";
